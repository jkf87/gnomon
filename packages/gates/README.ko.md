...
교육학에서는 이걸 **백워드 설계(Backward Design)** 라고 합니다. "수업부터 하고 나중에 채점 기준을 정하는" 게 아니라, **"어떤 증거를 보면 합격인지"를 먼저 정하고 수업을 설계**하는 방법이죠.
...
2. 각 항목마다 **게이트(gate, 관문)** 를 자동으로 만듭니다. 게이트 하나에는 이런 게 들어 있습니다:
   ...
   - **검증 예시 한 쌍** — "이런 건 합격, 이런 건 불합격"이라는 견본. 정답지처럼 게이트에 함께 저장해 두고, 채점기가 정답과 오답을 제대로 구분하는지 확인하는 데 써요.
...
- **최종 점수**는 심판들 점수의 **평균**으로 정합니다(불확실성도 같은 방식으로 평균).

이때 처음 심판의 점수와 표는 빠지고, 새로 모인 심판들의 결과만 반영됩니다. 이래도 끝까지 사람은 안 나섭니다. 사람 없이 자동으로 굴러가는 게 이 도구의 목표거든요.
...
문장이 바뀌면 이런 일이 일어납니다: **이전 문장으로 만들어 둔 게이트는 새 문장과 지문이 안 맞아서 더 이상 쓰이지 않습니다.** 새 문장에는 새 게이트가 적용돼요 — 새로 자동 생성하거나, 그 전까지는 비상용 기본 게이트(default)가 대신 들어갑니다. 어느 쪽이든 **바꾼 문장이 예전 채점 결과를 슬쩍 물려받는 일은 없습니다.** 같은 입력에는 언제나 같은 게이트 — 몰래 기준을 낮출 수 없어요.
...
| 합격 여부(compliance) | 심판의 예/아니오 답 — 기준을 만족하는가? |
