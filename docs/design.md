# openclaw-ouroboros — Design Spec
...
Make Ouroboros conveniently usable from inside OpenClaw, distributed as its own GitHub repo, and verified end-to-end on the user's machine. The integration must:

1. Expose Ouroboros via **two entry points** in OpenClaw: a slash skill and an MCP server.
2. Install with **one command** (no `curl | bash`).
3. Be packaged in a **separate repo** (`jkf87/openclaw-ouroboros`) decoupled from the heavy ouroboros-ai Python distribution.
...
- Replacing the existing `feat/openclaw-runtime` adapter inside the `ouroboros` repo. That work stays where it is and lands via its own PR; this repo is the OpenClaw-side companion.
- ClawHub registry submission in v1. Structure must allow it later, but it is not in scope now.
- Bundling Ouroboros' Python source. Ouroboros is invoked via `uvx --from ouroboros-ai[mcp]`; this repo only carries the OpenClaw entry points.
...
```
┌─ openclaw-ouroboros (this new repo) ──────────────┐
│  skills/ouroboros/SKILL.md       ← slash skill    │
│  src/openclaw_ouroboros/         ← uvx installer  │
│  prompts/                        ← agent-facing   │
│  verification/                   ← 95% gate       │
└──────────────────────┬────────────────────────────┘
                       │ depends on (uvx)
┌──────────────────────▼────────────────────────────┐
│ ouroboros-ai (PyPI / git, jkf87/ouroboros)        │
...
Boundaries:
- **openclaw-ouroboros** owns: OpenClaw skill files, MCP entry merge logic, install/uninstall/doctor commands, agent prompts, verification harness.
- **ouroboros-ai** owns: workflow engine, MCP server, runtime adapters (including `OpenClawCliRuntime`).
...
### 4.1 Slash skill — `~/.openclaw/skills/ouroboros/SKILL.md`

Korean + English natural-language triggers. Slash commands mirror the existing `ooo` skill set already shipped inside `~/ouroboros/skills/`:

| Skill | Purpose |
|-------|---------|
| `/ouroboros:setup` | Run `ouroboros setup --runtime openclaw` and verify |
| `/ouroboros:interview` | Start a spec-first interview |
| `/ouroboros:run` | Execute a workflow (`ouroboros run workflow ...`) |
| `/ouroboros:status` | Inspect execution status |
| `/ouroboros:cancel` | Cancel a running execution |

The SKILL.md body wraps `uvx --from ouroboros-ai[mcp] ouroboros <subcommand>` calls and parses the JSON envelope so the agent can stream user-facing text.

### 4.2 MCP server — `~/.openclaw/mcp/claude-mcp-config.json`

Add an `ouroboros` entry alongside the existing `openclaw` entry:
...
Merge must be **idempotent**: re-installing must not duplicate the entry; uninstalling must remove only the `ouroboros` key without disturbing siblings.
...
| Command | Behavior |
|---------|----------|
| `install` | Stage SKILL.md, merge MCP entry, write `~/.ouroboros/config.yaml` (runtime_backend=openclaw), warm uvx cache, print diagnostics |
| `uninstall` | Reverse all of the above; never delete user-authored skills/MCP entries |
| `doctor` | Verify each artifact is present and Ouroboros responds via uvx |
...
├── pyproject.toml                # uvx entry point: openclaw-ouroboros
├── src/openclaw_ouroboros/
│   ├── __init__.py
│   ├── cli.py                    # install / uninstall / doctor
│   ├── installer.py              # idempotent merge logic
│   ├── paths.py                  # OpenClaw / Ouroboros path resolution
│   └── skill_template/
│       └── SKILL.md              # copied verbatim into ~/.openclaw/skills/ouroboros/
├── prompts/
...
| `prompts/install.md` | OpenClaw end-user (paste into Claude Code session) | "Install Ouroboros for me, run doctor, report back" — wraps the uvx command with diagnostic and rollback context. |
...
| 5 | Ouroboros compat | SKILL.md commands match `feat/openclaw-runtime` adapter contract; MCP entry uses `ouroboros-ai[mcp]` extra; config.yaml schema matches `~/ouroboros/docs/runtime-guides/openclaw.md`. |
...
1. New public GitHub repo `jkf87/openclaw-ouroboros` with the layout in §6.
...
5. (Adjacent, not in this repo) PR or branch hygiene for `~/ouroboros@feat/openclaw-runtime` so the upstream side is presentable.

## 10. Deferred to autopilot

Choices the autopilot may make without further consultation:

- License file content (default: MIT, matching ouroboros-ai)
...
- Automatic upgrade flow (`openclaw-ouroboros upgrade`)
...
- **ouroboros adapter drift**: `feat/openclaw-runtime` is unmerged upstream; if it gets rebased, the SKILL.md command surface might shift. Mitigation: §8 dim 5 makes this an explicit reviewer concern.
